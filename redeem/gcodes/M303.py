  ...
  def execute(self, g):
    ...
    if tuner.run():
      ...
      self.printer.send_message(g.prot, "Max temp: {}, Min temp: {}, Ku: {}, Pu: {}".format(
          tuner.max_temp, tuner.min_temp, tuner.Ku, tuner.Pu))
      self.printer.send_message(g.prot, "Kp: {}, Ti: {}, Td: {}".format(
          tuner.Kp, tuner.Ti, tuner.Td))
      if not write_to_cfg:
        ...
        self.printer.send_message(g.prot, "PID settings for {} ({}) saved in local.cfg".format(
            heater.name, heater.input.name))
